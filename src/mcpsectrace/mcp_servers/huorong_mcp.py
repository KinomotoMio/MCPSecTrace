...
# 病毒查杀功能对应的图片
QUICK_SCAN_BUTTON_IMAGE = 'D:/FTAgent-master/tag_image/huorong/huorong_quick_scan_button_me.png'
PAUSE_BUTTON_IMAGE = 'D:/FTAgent-master/tag_image/huorong/huorong_pause_button_me.png'
SCAN_COMPLETE_IMAGE = 'D:/FTAgent-master/tag_image/huorong/huorong_complete_button_me.png'

# 查看隔离区
QUARANTINE_BUTTON_IMAGE = 'D:/FTAgent-master/tag_image/huorong/huorong_quarantine_button_me.png'
MAXIMIZE_BUTTON_IMAGE = 'D:/FTAgent-master/tag_image/huorong/huorong_maximize_button_me.png'
...
EXPORT_LOG_BUTTON_IMAGE = 'D:/FTAgent-master/tag_image/huorong/huorong_export_log_button_me.png'
FILENAME_INPUT_BOX_IMAGE = 'D:/FTAgent-master/tag_image/huorong/huorong_filename_input_box_me.png'
...
EXPORT_COMPLETE_IMAGE= 'D:/FTAgent-master/tag_image/huorong/huorong_export_complete_me.png'
...
def find_and_click(image_filename, confidence_level, timeout_seconds, description):
    ...
    """
    img_loc = find_image_on_screen(image_filename = image_filename, confidence_level=0.6, timeout_seconds=15, description =description)
    if img_loc:
...
def ret2_top_page():
    ...
    COMPLETE_BUTTON_IMAGE = 'D:/FTAgent-master/tag_image/huorong/huorong_complete_button_me.png'
    img_loc = find_image_on_screen(COMPLETE_BUTTON_IMAGE, confidence_level=0.6, timeout_seconds=15, description="完成按钮")
    if img_loc:
...
def scan_virus():
    ...
    # 步骤2：点击“快速查杀”按钮
    img_loc = find_image_on_screen(QUICK_SCAN_BUTTON_IMAGE, confidence_level=0.6, timeout_seconds=15, description="快速查杀按钮")
    if img_loc:
    ...
    # 步骤3：检测是否正在查杀
    if find_image_on_screen(PAUSE_BUTTON_IMAGE, confidence_level=0.6, timeout_seconds=15, description="暂停按钮"):
    ...
    while time.time() - start_time < interval:
        img_loc = find_image_on_screen(SCAN_COMPLETE_IMAGE, confidence_level=0.6, timeout_seconds=15, description="快速查杀完成")
        if img_loc:
...
def get_security_log():
    ...
    # 1.点击首页的安全日志图标
    if not find_and_click(SECURITY_LOG_IMAGE, confidence_level=0.6, timeout_seconds=20, description="安全日志"):
        return "未能找到安全日志图标。"
    time.sleep(SLEEP_TIME_MEDIUM) 
    ...
    if not find_and_click(EXPORT_LOG_BUTTON_IMAGE, confidence_level=0.9, timeout_seconds=15, description="导出日志按钮"):
        return "未能找到导出日志按钮，或点击失败。"
    time.sleep(SLEEP_TIME_MEDIUM)
    # 检查是否点击成功
    if not find_image_on_screen(SAVE_MARK_IMAGE, confidence_level=0.6, timeout_seconds=15, description="另存为标记"):
    ...
    if not find_and_click(FILENAME_INPUT_BOX_IMAGE, confidence_level=0.6, timeout_seconds=15, description="文件名输入框"):
    ...
    if not find_and_click(SAVE_BUTTON_IMAGE, confidence_level=0.6, timeout_seconds=15, description="保存按钮"):
        return "未能找到保存按钮。"
    debug_print(f"安全日志导出流程执行完毕，请查看文件D:/Desktop/{current_time_str}.txt。")
    time.sleep(SLEEP_TIME_MEDIUM)  # 等待保存操作完成

    # 7.检查是否导出成功
    if not find_image_on_screen(EXPORT_COMPLETE_IMAGE, confidence_level=0.6, timeout_seconds=15, description="导出完成标志"):
