...
def scan_virus():
    ...
    debug_print("hrkill软件已启动，请确保处于首页，否则后续可能执行失败。") # 能否显示在mcp上
    
    # 2. 点击'开始扫描按钮'
    debug_print(f"[Step 2] 点击'开始扫描按钮'")
    if not find_and_click(START_SCAN_BUTTON_IMAGE, confidence_level=0.8, timeout_seconds=15, description="开始扫描按钮"):
        return "[ERROR] 未能找到'开始扫描按钮'，或点击按钮失败，请查看最新日志溯源。"
    time.sleep(SLEEP_TIME_LONG)

    # 3. 检测是否正在查杀病毒
    debug_print(f"[Step 3] 检测是否正在查杀病毒")
    if find_image_on_screen(PAUSE_BUTTON_IMAGE, confidence_level=0.8, timeout_seconds=15, description="查杀暂停按钮"):
    ...
    start_time = time.time()
    img_loc = find_image_on_screen(SCAN_COMPLETE_IMAGE, confidence_level=0.8, timeout_seconds=interval, description="扫描完成")
    if img_loc:
...
def main():
    ...
    parser.add_argument('--hrkill-path', type=str, required=True, help='hrkill软件的完整路径')
